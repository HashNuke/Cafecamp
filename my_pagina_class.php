	...
	function navigation($separator = " | ", $css_current = "", $back_forward = false) {
		...
		if ($curr_pages <= $all_pages && $curr_pages >= 0) {
			...
			if($all_pages >= 1) {
				...
				$navi_string = ($curr_pages > 0) ? "<a href=\"".$_SERVER['PHP_SELF']."?q=".$_REQUEST['q']."&".$var."=".$backward.$this->rebuild_qs($var)."\">".$this->str_backward."</a>&nbsp;" : $this->str_backward."&nbsp;";
				if (!$back_forward) {
					for($a = $start + 1; $a <= $end; $a++){
						$theNext = $a - 1; // because a array start with 0
						if ($theNext != $curr_pages) {
							$navi_string .= "<a href=\"".$_SERVER['PHP_SELF']."?q=".$_REQUEST['q']."&".$var."=".$theNext.$this->rebuild_qs($var)."\">";
							$navi_string .= $a."</a>";
				...
				$navi_string .= ($curr_pages < $all_pages) ? "&nbsp;<a href=\"".$_SERVER['PHP_SELF']."?q=".$_REQUEST['q']."&".$var."=".$forward.$this->rebuild_qs($var)."\">".$this->str_forward."</a>" : "&nbsp;".$this->str_forward;
			}
		}
		return $navi_string;
	}
